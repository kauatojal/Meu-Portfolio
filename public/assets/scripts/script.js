document.addEventListener('DOMContentLoaded', function () {
    ...
    if (carousel && carouselContainer) {
        ...
        document.getElementById('projects-prev')?.addEventListener('click', () => moveToProject(-1));
        document.getElementById('projects-next')?.addEventListener('click', () => moveToProject(1));
        measureCarousel();
...
(function setupPremiumMotion(){
    ...
    const context = window.gsap.context(() => {
        ...
        if (orbit) {
            const orbitTimeline = window.gsap.timeline({ scrollTrigger: { trigger: '.home', start: 'top top', endTrigger: '.projects', end: 'bottom top', scrub: 1.2 } });
            orbitTimeline.to(orbit, { x: -90, y: 220, scale: .76, rotation: 82, ease: 'none' }).to(orbit, { x: 45, y: 480, scale: .56, rotation: 155, ease: 'none' });
        }
